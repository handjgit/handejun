...
<head>
	...
	<script type="text/javascript">
		...
		function shenhe(userId){
			$.get{"${ctx}/platform/tuser/shenhe?id="+userId,function(result){  }};
		}
	</script>
</head>
<body class="gray-bg">
	...
	<div class="row">
	<div class="col-sm-12">
	<form:form id="searchForm" modelAttribute="tuser" action="${ctx}/platform/tuser/list?type=3" method="post" class="form-inline">
	...
	<div class="col-sm-12">
		<div class="pull-left">
			<%-- <shiro:hasPermission name="platform:tuser:add">
				<table:addRow url="${ctx}/platform/tuser/form" title="学生信息"></table:addRow><!-- 增加按钮 -->
			</shiro:hasPermission>
			<shiro:hasPermission name="platform:tuser:edit">
			    <table:editRow url="${ctx}/platform/tuser/form" title="学生信息" id="contentTable"></table:editRow><!-- 编辑按钮 -->
			</shiro:hasPermission> --%>
		...
		<c:forEach items="${page.list}" var="tuser">
			...
			<%-- 	<td>
				...
				<td>
					...
					<shiro:hasPermission name="platform:tuser:edit">
    					<a href="${ctx}/platform/tuser/shenhe?id=${tuser.id}"  class="btn btn-success btn-xs" ><i class="fa fa-edit"></i> 审核通过</a>
    				</shiro:hasPermission>
    				<shiro:hasPermission name="platform:tuser:del">
						<a href="${ctx}/platform/tuser/delete?id=${tuser.id}" onclick="return confirmx('确认要删除该商家信息吗？', this.href)"   class="btn btn-danger btn-xs"><i class="fa fa-trash"></i> 删除</a>
					</shiro:hasPermission> 
				</td>
			</tr>
		</c:forEach>
