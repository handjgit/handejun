...
//				ChatServerPool.sendMessageToUser(conn,  sender+Constant._msg_+c.getUserid2()+Constant._msg_+c.getMsg()+Constant._msg_+UserUtils.getByLoginName(userid1).getPhoto()+Constant._msg_+c.getType()+Constant._msg_+UserUtils.getByLoginName(userid1).getName()+Constant._msg_+c.getCreateDate().getTime());//向所某用户发送消息
			...
			if("group".equals(type)){//如果是群聊
				...
				for(LayGroupUser lgUser:layGroupUserlist){
					if(fromUser.equals(lgUser.getUser().getName())){
						continue;//群聊消息不发给自己。
					}
					WebSocket toUserConn = ChatServerPool.getWebSocketByUser(lgUser.getUser().getName());
					...
					chat.setUserid2(lgUser.getUser().getName());//群中所有信息获得者人员
					chat.setMsg(msg);
					chat.setCreateDate(new Date());
					chat.setType("group");  
					if(toUserConn != null){
						message = groupId+Constant._msg_+lgUser.getUser().getName()+Constant._msg_+msg+Constant._msg_+avatar+Constant._msg_+type+Constant._msg_+fromUsername+Constant._msg_+datatime;
